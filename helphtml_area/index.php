...
<?
	if($content)
	{
		...
		echo '<a class="gdhelpareachlink" href="/communication/" style="text-decoration: none">'.$parser->convertText($content).'</a>';
	}
	else
	{
...
<?if($arParams["PERMISSION"]>"R"):?>
<div class="gdhelphtmlareach" style="padding-top: 10px;"><a class="gdhelphtmlareachlink" href="<?=$GLOBALS["APPLICATION"]->GetCurPageParam("gdhelphtml=".$id."&edit=true", array("gdhelphtml", "edit"))?>"><?echo GetMessage("GD_HTML_AREA_CHANGE_LINK")?></a></div>
<?endif?>

<?else:?>

<form action="?gdhelphtml=<?=$id?>" method="post" id="gdf<?=$id?>">
<?
...
$LHE->Show(array(
	...
	'ctrlEnterHandler' => 'gdhelphtmlsave',
));
?>
	<input type="hidden" name="gdhelphtmlform" value="Y">
...
<a href="javascript:void(0);" onclick="return gdhelphtmlsave();"><?echo GetMessage("GD_HTML_AREA_SAVE_LINK")?></a> | <a href="<?=$GLOBALS["APPLICATION"]->GetCurPageParam(($arParams["MULTIPLE"]=="Y"?"dt_page=".$arParams["DESKTOP_PAGE"]:""), array("dt_page","gdhelphtml","edit"))?>"><?echo GetMessage("GD_HTML_AREA_CANCEL_LINK")?></a>
<?endif?>